...
var (
	...
	vdfileModbusDelay    vdfile.ConfigModbus
)
...
func init() {
	...
	configModbusDelay, _ := vdfile.DecodeVDFileModbus(FILE_MODBUS)
	configModbusDelay.Delay = "3s"
	vdfileModbusDelay = configModbusDelay
}
...
func TestRunWithDelays(t *testing.T) {
	...
	tests := []struct {
		...
		{"state check", 3 * time.Second, []byte("CUR?\r\n"), []byte("CUR 300\r\n")},
		{" check", 4 * time.Second, []byte("PSI?\r\n"), []byte("PSI 3.30\r\n")},
...
func TestRunWithDelaysModbus(t *testing.T) {
	t.Parallel()
	if testing.Short() {
		t.Skip("skipping test in short mode.")
	}
	defer setupModbusTestCase(t, ADDR7, vdfileModbusDelay)()
	// connect to server
	conn, err := net.Dial("tcp", ADDR7)
	if err != nil {
		t.Fatalf("could not connect to to server: %v\n", err)
	}
	defer conn.Close()
	tests := []struct {
		...
		{"read temp", 3 * time.Second, []byte{0x00, 0x01, 0x00, 0x00, 0x00, 0x06, 0x01, 0x03, 0x00, 0x05, 0x00, 0x01}, 11, []byte{0x00, 0x01, 0x00, 0x00, 0x00, 0x05, 0x01, 0x03, 0x02, 0x00, 0x14}},
		{"read not param", 0, []byte{0x00, 0x01, 0x00, 0x00, 0x00, 0x06, 0x01, 0x03, 0x00, 0x05, 0x00, 0x77}, 11, []byte{0x00, 0x01, 0x00, 0x00, 0x00, 0x05, 0x01, 0x03, 0x02, 0x00, 0x00}},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// set timeout for reading data
			...
			if !(tt.dur < elapsed && elapsed < (tt.dur+time.Millisecond)) {
				t.Errorf("exp delay around: %v got: %v\n", tt.dur, elapsed)
			}
		})
	}
