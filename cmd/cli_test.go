...
func TestGetParameterType(t *testing.T) {
	tests := []struct {
		...
		{"get pressure type", "pressure", "float64\n", API_ADDR_MODBUS},
		{"get state type", "state", "uint8\n", API_ADDR_MODBUS},
...
func TestGetDelayStream(t *testing.T) {
	tests := []struct {
		name  string
		input string
		exp   string
		...
		{"wrong api addr", "get_psi", `Error: Get "http://127.0.0.1:7878/delay/stream/get_psi": dial tcp 127.0.0.1:7878: connect: connection refused` + "\n", "127.0.0.1:7878"},
...
func TestGetDelayModbus(t *testing.T) {
	tests := []struct {
		...
		{"get delay", "3s\n", API_ADDR_MODBUS},
		{"wrong api addr", `Error: Get "http://127.0.0.1:7878/delay/modbus": dial tcp 127.0.0.1:7878: connect: connection refused` + "\n", "127.0.0.1:7878"},
		{"wrong api addr format", "Error: wrong HTTP address\n", "127.test"},
	}
...
func TestSetDelayWrong(t *testing.T) {
	tests := []struct {
		...
		{"wrong api addr", "get_psi 10s", `Error: Post "http://127.0.0.1:7878/delay/stream/get_psi/10s": dial tcp 127.0.0.1:7878: connect: connection refused` + "\n", "127.0.0.1:7878"},
