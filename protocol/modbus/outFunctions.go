...
func (p *Parser) GenerateReadCoilsResponse(frame TCPFrame, resps []protocol.Response) ([]byte, *Exception) {
	err := updateSingleBitsMemory(resps, p.paramsAddrs, p.coilTable)
	if err != nil {
...
func (p *Parser) GenerateReadDIsResponse(frame TCPFrame, resps []protocol.Response) ([]byte, *Exception) {
	err := updateSingleBitsMemory(resps, p.paramsAddrs, p.diTable)
	if err != nil {
...
func (p *Parser) GenerateReadHoldingRegistersResponse(frame TCPFrame, resps []protocol.Response) ([]byte, *Exception) {
	res := &Success
	err := updateRegisterMemory(resps, p.paramsAddrs, p.holdRegTable)
	if err != nil {
...
func (p *Parser) GenerateReadInputRegistersResponse(frame TCPFrame, txs []protocol.Response) ([]byte, *Exception) {
	res := &Success
...
func updateSingleBitsMemory(resps []protocol.Response, params map[string]memory.Memory, memoryTable []byte) error {
	for _, resp := range resps {
		for name, val := range resp.Params {
...
func updateRegisterMemory(resps []protocol.Response, params map[string]memory.Memory, memoryMap [][]byte) error {
